...
CREATE OR REPLACE PROCEDURE registrarVeterinario(curp VARCHAR,idEstetica INT, 
	...
	AS $$
	BEGIN
		IF (apellidoM ~* '[abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ]' &&
			apellidoP ~* '[abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ]' &&
			nombre ~* '[abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ]') THEN
